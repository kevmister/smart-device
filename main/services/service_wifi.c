...
void service_wifi_event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data){
	...
	if(event_base == IP_EVENT){
		if(event_id == IP_EVENT_STA_GOT_IP){
			...
			ESP_LOGI(TAG, "Station got address");
		}
	}
}
...
service_config_error_t service_wifi_config(cJSON *service_config){
	...
	int sta_enabled = cJSON_GetObjectItem(service_config, "sta_enabled")->valueint;
	int ap_enabled = cJSON_GetObjectItem(service_config, "ap_enabled")->valueint;
